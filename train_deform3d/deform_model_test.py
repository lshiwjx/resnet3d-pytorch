...
optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.weight_decay_ratio)
...
model.cuda()
model.train()
clip, labels = Variable(clip).cuda(), Variable(labels).cuda()
...
torch.save(layers[0], 'off_tensor_b31')
torch.save(layers[1], 'off_tensor_b32')
print('none')
